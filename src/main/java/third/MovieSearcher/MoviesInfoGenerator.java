			...
			for (File childFile : subFiles) {
				if (childFile.isFile()) {
					...
					if (moviesSearchResult != null) {
						...
						//                      ToDo: на будущее: соединять строки лучше с помощью StringBuilder. но для наших простых программ пока можно оставить как есть для более удобного чтения
						//                      вещи которые нужно доделать в будущем можно описывать в коментариях и помечать меткой todo
					} else {
